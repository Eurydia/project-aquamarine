...
export const StyledSelect: FC<StyledSelectProps> = (props) => {
  const { label, value, optionToIcon, onChange, options, disabledOptions } =
    props;
  ...
  const renderedOptions = _options.map((opt) => (
    <MenuItem key={opt} value={opt}>
      <ListItemIcon>
        <img alt={opt} src={optionToIcon(opt)} />
      </ListItemIcon>
      <ListItemText>{opt}</ListItemText>
    </MenuItem>
  ));
  const renderedDisableOptions = disabledOptions.map((opt) => (
    <MenuItem disabled key={opt} value={opt}>
      {opt}
    </MenuItem>
  ));
